...
function customorderchangestatus(){ 
	extract($_POST);
	$result=$this->db->query("UPDATE customorder SET status = '$value'  WHERE uniqueid = '$id'");
	return $result;	
}
...
{
$pid=$_POST['pdtid'];
$qry = $this->db->query("SELECT * FROM fabric where id='$pid'");
$res = $qry->fetch_array()[3];
if($res==1){
	$qry2 = $this->db->query("UPDATE fabric SET status=2 WHERE id='$pid'");
}
else if($res==2){
	$qry2 = $this->db->query("UPDATE fabric SET status=1 WHERE id='$pid'");
}
return $qry2;
}

function availfabcheck()
{
	extract($_POST);
	if(isset($value)) {
